...
pickle.dump(transaction_table, output)
...
for group in transaction_table:
	...
	'''if group == 3702:
		var = Series(data = normalized_date_quantities, index = len(normalized_date_quantities))
		plt.hist(var)
		plt.show()
		break'''
	...
	if len(normalized_date_quantities) > CONFIDENCE_THRESH:
		...
		transition_models.append((len(normalized_date_quantities), numpy.std(normalized_date_quantities),numpy.mean(normalized_date_quantities),group,transaction_table[group][0][2]))
transition_models.sort()
pickle.dump(transition_models, output)
